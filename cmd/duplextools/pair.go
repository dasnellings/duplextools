...
func runPair(args []string) {
	...
	strictPosMatching := pairFlags.Bool("strictPosMatching", false, "For a read to be included in a read family, the boundaries of both reads in a pair (start of fwd read/end of rev read) must exactly match read family.")
