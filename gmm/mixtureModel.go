...
func runMixtureModel(expectationFunc func(*MixtureModel, int), data []float64, k int, pulsePeriod int, maxIterations, maxResets int, mm *MixtureModel) (converged bool, iterationsRun int) {
	...
	for iterationsRun = 0; resets < maxResets && !converged; iterationsRun++ {
		...
		// M step
		maximization(mm)
		//fmt.Println(mm.Means, mm.Stdev, mm.posteriorsSum, mm.LogLikelihood)
		...
		for i := 0; i < mm.K; i++ {
			if mm.Stdev[i] == 0 {
				mm.Stdev[i] = 1e-15 // rescue NaN for zero variance gaussian that explains a significant amount of data
			}
			switch {
			case mm.Stdev[i] < 1e-04 && mm.posteriorsSum[i] < 3: // most likely caused by overfit of gaussian to a single data point
				//fmt.Println("stdev reset", mm.Means, mm.posteriorsSum)
				fallthrough
			case converged && mm.Stdev[i] > 6:
				fallthrough
			case converged && mm.Weights[i] < 0.1: // they asked for 2 gaussians, reset and see if you can get better answer
				//fmt.Println("weight reset", mm.Means, mm.posteriorsSum)
				resets++
				initMixtureModel(data, k, maxIterations, mm)
				iterationsRun = 0
				prevLogLikelihood = 0
